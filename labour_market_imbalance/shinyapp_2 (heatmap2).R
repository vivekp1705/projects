...
sjd2_reordered  <- sjd2_reordered %>% 
  mutate(Region = case_when(country %in% c('Australia', 'New Zealand') ~ "Oceania", 
                           country %in% c('Canada', 'Mexico', 'United States', 'Chile') ~ "Americas", 
                           country %in% c('European Union') ~ "European Union",
...
ui_tabs <- 
  fluidPage (titlePanel(title = div(img(src = "C:/Users/bluff/OneDrive - Burgundy School of Business/BSB/Shiny/assignment/shinyassignment/OECD-social-sharex.jpg"), 
             ...
             fluidRow(navbarPage(title = "Select the type of analysis",
                                 fluidRow(column(2, selectInput("region", 
                                                                ...
                                                                selected = "Europe - EU", multiple = F), #a region filter at the page level to synchronize it with all the tabs
                                 ),
                     ...
                     indicate greater balance."  ),
                                ...
                                #--------------- Sub Category Level Skill ----------------
                                            tabPanel(title = "Sub Categories of Top Level Skills",
                                                     fluidRow(column(2, selectInput ("level2", "Select skill level 2", choices = sjd2$skill2, selected
                                                                              = "All", multiple = F),
                                                              )
                      ...
                      indicate greater balance."))),
                                                     fluidRow(column(8, d3heatmapOutput("heatmap2", 
                                                                                        height="350px", # changes the height of the table of data (350 seems good)
                                                                                        width="1400px")),
                                                              column(4, dataTableOutput('stats2')# changes the width of the table of data (1400 seems good)
                                                              ) 
                                                     ),
...
server  <- 
  function(input, output) {
    ...
    output$stats1 = DT::renderDT(
      datatable(sjd1_stats()[ , c('skill1', 
                ...
                options = list(paging = FALSE, # short table so no need for paging or searching
                               searching = FALSE),
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  font-size:200% ;','Table1: Top level skill statistics in OECD Countries')
      )
    )
    ...
    output$column_plot = renderPlotly(({
      ggplotly(sjd2_reordered %>% 
                 filter(skill1 == input$level1 & skill2 == "Overall" & region == input$region & country != "OECD" & country != "European Union") %>%
                 ggplot2::ggplot(aes(x = reorder(country, -value), y = value, fill = Region,
                                     text = paste0("Country: ", {country}, '<br>',
                                                   "Region: ", {region}, '<br>',
                                                   "Level of Mismatch: ", {value})))+ #text for configuring hover info
                 geom_col()+
              geom_hline(yintercept = as.numeric(sjd2_reordered %>% 
                          filter(skill1 == input$level1 & skill2 == "Overall" & country == "OECD" & region != "All") %>% select(value)), 
                          linetype = "dashed", color="#30D5C8",  hoverinfo = "OECD")+
    ...
    output$stats2 = renderDataTable(
      datatable(sjd2_stats()[ , c('skill2', 
                ...
                options = list(paging = FALSE, # short table (max 11 rows) so no need for paging or searching
                               searching = FALSE),
                ,
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  font-size:200% ;','Table2: Sub category level skill statistics in OECD Countries')
      )
    )
