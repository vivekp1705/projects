...
ui_tabs <- 
  fluidPage (titlePanel(title = div(img(src = "OECD-social-sharex.jpg"), "Skills Imbalance on the Labour Market in OECD Countries")), 
...
#----------- tab1 -----------
  fluidRow(navbarPage(title = "Select the type of Analysis",
                      fluidRow(column(2, selectInput("region", 
                                                     "Region", 
                                                     choices = append("All", unique(sjd1[sjd1$region != '', ]$region)), 
                                                     selected = "Europe - EU", multiple = F), 
                      )  
                      ),
                      navbarMenu("Regional Analysis", 
                     ...
                     fluidRow (
                               column (12, 
                                       d3heatmapOutput("heatmap1", 
                                                         ...
                                                         width="1900px" # changes the width of the table of data (1900 seems good)
                                                      ),
...
#--------- tab 2---------
                      navbarMenu(title = "Skill Analysis",
                                 
           #---------- Skill 1 -----------
            tabPanel(title = "Top level skills",
                     fluidRow(column(2,selectInput ("level0", 
                                                    "Top-Level Skill", 
                     ...
                     fluidRow(column(8, plotlyOutput("column_plot")),
                      ...
                      indicate greater balance."))),
                              fluidRow(column(8, d3heatmapOutput("heatmap2", 
                                                                 height="350px", # changes the height of the table of data (350 seems good)
                                                                 width="1400px")),
                                              column(4, dataTableOutput('stats2')# changes the width of the table of data (1900 seems good)
                                                ) 
                                              ),
                              )
                                           ),
           #---------- Skill 2 -----------
            tabPanel(title = "Sub categories of top level skills",
                     fluidRow(column(2, selectInput("level1", "Select skill level 1", choices = sjd2$skill1, 
                                                 selected = "All", multiple = F)),
                              column(2, selectInput ("level2", "Select skill level 2", choices = sjd2$skill2, selected
                                                    = "All", multiple = F),
                                     #selectInput("regions", "Select the region", choices = sjd2$region, multiple = F )
                                     )
                              ),
...
server  <- 
  function(input, output) {
    #------------------------------- Skill - (sub tab) skill 2 ---------------------
    skill1 <- reactive({
      filter(sjd2_reordered, skill1 == input$level0)
    })
    
    observeEvent(skill1(),{
     skill1 <- unique(skill1()$skill1)
     updateSelectInput(inputId = "level1", choices = skill1)
    })
    
    #skill1 <- reactive({
     # filter(sjd2_reordered, skill1 == input$level1)
    #})
    observeEvent(skill1(),{
    ...
    output$plot4 <- renderPlotly({
      ggplotly(sjd2_oecd %>%
                 filter(skill2 == input$level2 & region == input$region) %>% 
  ...
  #      scale_fill_manual(values = c("#D1EAF0", "#0077B6", "#0077B6", "#ADD8E6", "#82EEFD", "#87CEEB"))+ #"#90E0EF", "#0077B6", "#61A5C2"))+
        labs(title = paste0("Labour market comparison at the sub category level of ", input$level2, " in ", input$region, 
                            '<br>',
                            '<sup>',
                            plotData()[which.max(plotData()$value),4], " has the highest shortage of ", input$level0, " in ", input$region),  
             x = 'Countries',
    ...
    #----Region----Skills-----Bar 1 and 2------
    
    #skill 1&2 reactive df
    rs12b <- reactive({
      sjd2_reordered %>%
        filter(region == input$region & skill2 != "Overall") %>% 
    ...
    ggplotly(rs12b() %>% 
        ...
        coord_flip()+
        #  facet_wrap(~skill1, )+
        labs(title = paste0("Labour market at the sub-category level in ", input$country,
                            '<br>',
                            '<sup>',
                            'has the highest shortage of ', rs12b()[which.max(rs12b()$value),2], 
                            " and highest supply of ", rs12b()[which.min(rs12b()$value),2], ' skills', '<br>'),  
        ...
        theme(panel.background = element_blank(),
              ...
              plot.subtitle = element_text(size = 10, hjust = 0.5, face = "italic")),
        tooltip = "text") %>% 
        layout(annotations = list(x = 1, y = -0.025, text = "Source - OECD:2015", showarrow = F, xref='paper', yref='paper', 
                                  xanchor='right', yanchor='auto', xshift=0, yshift=0,
                                  font=list(size=9)))
    })
    ...
    #----------------------------------- Region Sub Tab (bar, boxplot, table and heatmap) --------------------------
    
    # calculate reactive data table of statistics to display
    sjd2_stats = reactive({
      sjd2_reordered %>% 
        #select(-region) %>% 
        filter(skill1 == input$level0) %>%
        filter(region == input$region) %>% 
        group_by(skill2) %>% 
        summarize(skill2_max = sprintf("%.2f", round(max(value, na.rm = TRUE), 2)), # changes display formatting so all values show two decimal places
                  skill2_min = sprintf("%.2f", round(min(value, na.rm = TRUE), 2)),
    ...
    output$boxplot <- renderPlotly({
      ggplotly(sjd2_oecd %>%
                 filter(skill1 == input$level0 & skill2 == "Overall" & region != "") %>% 
                 ggplot(aes(x = region, y = value, fill = region)) +
                 geom_boxplot()+
                 stat_summary(fun = "mean", geom = "point", shape = 10, size = 2, color = "white")+
                 theme_light()+
                 labs(title = paste0("Range and average of ", input$level0, 
                                     '<br>',
                                     '<sup>',
                                     "skill mismatch level across all regions"),
                      x = 'Countries',
    ...
    titleData <- reactive({
      sjd2_oecd %>% 
        filter(skill1 == input$level0 & skill2 == "Overall") %>% 
        #filter(skill2 == input$level2) %>% 
        #filter(region!= "All")
        filter(region == input$region & country != "OECD" & country != "European Union")
    })
    ...
    output$column_plot = renderPlotly(({
      ggplotly(sjd2_oecd %>% 
                 filter(skill1 == input$level0 & skill2 == "Overall" & region == input$region) %>%
                 ggplot2::ggplot(aes(x = reorder(country, -value), y = value, fill = color,
                                     text = paste0("Country: ", {country}, '<br>',
                                                   "Region: ", {region}, '<br>',
                                                   "Level of Mismatch: ", {value})))+ #text for configuring hover info
                 geom_col()+
                 labs(title = paste0("Labour market top level skill comparison of ", input$level0, " in ", input$region, #reactive skill and region names
                                     '<br>',
                                     '<sup>',
                                     #for reactive name of region based on highest skill value by region
                                     titleData()[which.max(titleData()$value),4], " has the highest shortage of ", input$level0, " in ", input$region), #reactive name of the country having highest shortage
                      x = 'Countries',
                      y = "Skill Mismatch Indicator",
                      caption = "Source: OECD, 2015") + 
