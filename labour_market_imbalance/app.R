   ...
   titlePanel(title=div(img(src="OECD-social-sharex.jpg", height=50, width=100), 
    ...
    theme = shinytheme("cerulean"), #theme for the dashboard
             ...
             fluidRow(navbarPage(title = "Select the type of analysis",
                                 ...
                                 column(2, ),
                                 column(2, box(title = "Countries", uiOutput("txt1"), height = 50, width= 120, background = "navy"), # KPI indicating number of countries in the region
                                        ),
                                 column(2, box(title = "Top-Level Skills", uiOutput("txt2"), height = 50, width= 120), # KPI indicating total number of top level skills
                                        ),
                                 column(2, box(title = "Sub-Categories", uiOutput("txt3"), height = 40, width = 120), # KPI indicating number of sub categorical skills in the top level skill
                                 ),
                                 ),
                                            ...
                                            tabPanel(title = "By Country",
                                                     ...
                                                     fluidRow(column(6, plotlyOutput("plot1",
                                                              ...
                                                              column(6, plotlyOutput("plot2",
                                                                     ...
                                                                     div(style = "height:17px"), #to align all the 3 tables with each other
...
server  <- 
  function(input, output, session) {
    ...
    output$txt1 <- renderUI({
      HTML(paste0("<b><strong>", "<font face = Verdana; size = 2; font-weight = bold; color = navy>",
    ...
    output$txt4 <- renderUI({
      HTML(paste0("<b><strong>", "<font face = Verdana; size = +1; font-weight = bold; color = #000000>", 
    ...
    output$stats1 = DT::renderDT(
      datatable(sjd1_stats()[ , c('skill1', 
                ...
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  
                                                  font-size:125% ;font-family: verdana ; font-weight: bold'
    ...
    output$heatmap1 = renderD3heatmap({
      d3heatmap(sjd1_matrix(),
                ...
                keysize = .40, # make legend 45% of the row height x column width
                ...
                print.values = F, # show values inside cells, change to TRUE if change mind
    ...
    output$column_plot = renderPlotly(({
      ggplotly(sjd2_reordered %>% 
                 ...
                 labs(title = paste0("Top-level skill comparison of ", input$level1, " in ", input$region, #reactive skill and region names
                                     '<br>',
                                     '<sup>',
                                     #for reactive name of region based on highest skill value by region
                                     titleData()[which.max(titleData()$value),4], " has the highest shortage"), #reactive name of the country having highest shortage
                      x = '',
                      y = "OECD Skills Imbalance Indicator",
        ...
        layout(annotations = list(x = 1, y = -0.30, text = "Source: OECD, 2015", showarrow = F, xref='paper', yref='paper', 
                                  xanchor='right', yanchor='auto', xshift=0, yshift=0,
                                  font=list(size=8))) #for displaying caption
    }))
    ...
    output$stats2 = renderDataTable(
      datatable(sjd2_stats()[ , c('skill2', 
                ...
                options = list(paging = FALSE, # short table (max 11 rows) so no need for paging or searching
                               searching = FALSE),
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  font-size:125% ;
  ...
  # column plot for comparing countries at sub category level 
    output$plot4 <- renderPlotly({
      ggplotly(sjd2_reordered %>%
         ...
         annotate("text", 
                  ...
                  size=4, 
                  color="black") +  # add label to horizontal line to show OECD average
         labs(title = paste0("Comparison at the subcategory level of ", input$level2, " in ", input$region, 
                             '<br>',
                             '<sup>',
                             ...
                             " has the highest shortage of ", input$level1),  
              x = '',
              y = "OECD Skills Imbalance Indicator",
              caption = "Source: OECD, 2015") + 
         theme(panel.background = element_blank(),
      ...
      layout(annotations = list(x = 1, y = -0.5, text = "Source: OECD, 2015", showarrow = F, xref='paper', yref='paper', 
                                xanchor='right', yanchor='auto', xshift=0, yshift=0,
                                font=list(size=9))) # for adding and adjusting caption
    })
      ...
      ggplotly(plot1_df() %>% 
                ggplot2::ggplot(aes(x = reorder(skill1, -value), y = value, fill = skill1,
                 ...
                 theme(panel.background = element_blank(),
                       ...
                       axis.text = element_text(size = 9),
                       legend.text = element_blank(),
                       plot.title = element_text(size = 14, hjust = 0.5, face = "bold", family = "Verdana"), # font size location and type adjustment 
    ...
    output$plot2 <- renderPlotly({
      ggplotly(plot2_df() %>% 
                 ...
                 geom_col()+
                 coord_flip()+
                 labs(title = paste0("Labour market for ", input$level1, " in ", input$country,
                 ...
                 theme(panel.background = element_blank(),
                       ...
                       plot.title = element_text(size = 14, hjust = 0.5, face = 'bold', family = "Verdana"), #adjusting font size, type and location
                       plot.subtitle = element_text(size = 10, hjust = 0.5, face = "italic", family = "Verdana")),
               tooltip = "text") %>% 
    ...
    output$stats3 = renderDataTable(
      datatable(sjd2_stats3()[ , c('skill1', 
                ...
                options = list(paging = FALSE, # short table (max 11 rows) so no need for paging or searching
                               searching = FALSE),
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  
                                                  font-size:125% ; font-family: verdana ; font-weight: bold', #adjusting font size, type for title 
    ...
    output$demand = renderDataTable(
      datatable(demand(),
                ...
                options = list(paging = FALSE, # short table (max 11 rows) so no need for paging or searching
                               searching = FALSE),
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  
                                                  font-size:125% ; font-family: verdana ; font-weight: bold'  #formating the caption to be displayed as the title with a selected font style 
                                                  ,paste0('Countries with Demand surplus for subcategory skills in ', input$region, ' for ', input$level1 
                                                  )) # for reacting to the selected region and skill in the title
        
      )%>% 
        DT::formatStyle(columns = c(1, 2, 3, 4), fontSize = '75%')
    )
    ...
    output$surplus = renderDataTable(
      datatable(surplus(),
                ...
                caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  
                                                  font-size:125% ;font-family: verdana ; font-weight: bold',
                                                  paste0('Countries with Supply surplus for subcategory skills in ', input$region, ' for ', input$level1 
                                                  )) # for reacting to the selected region and skill in the title
